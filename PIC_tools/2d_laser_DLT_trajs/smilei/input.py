...
position_ele = np.random.rand(3,40)
position_ele[0,:] = position_ele[0,:]*(foam_thick+subs_thick+cont_thick) + x_foam_start
position_ele[1,:] = (position_ele[1,:]-0.5)*waist*2 + y_spot
position_ele[2,:] = ne_f*dx*dy/nppc_ef*np.ones(40)

position_ionc = np.random.rand(3,20)
position_ionc[0,:] = position_ionc[0,:]*(cont_thick) + x_cont_start
position_ionc[1,:] = (position_ionc[1,:]-0.5)*waist*2 + y_spot
position_ionc[2,:] = ne_c*dx*dy/Z_c*nppc_ec*np.ones(20)
...
Species(
    name = 'ion_c_test',
    position_initialization = position_ionc,
    momentum_initialization = 'cold',
    mean_velocity = [0.],
    mass = A_c * 1836. ,
    charge = Z_c ,
...
Species(
    name = 'ele_test' ,
    position_initialization = position_ele,
    momentum_initialization = 'maxwell-juettner' ,
    temperature = [temp],
    mean_velocity = [0.],
    mass = 1.,
...
DiagFields(
    every =10*every_fs,
    fields = ['Bz', 'Ey','Rho_ele_f','Rho_ele_s','Rho_ele_c','Rho_ion_f','Rho_ion_s','Rho_ion_c']
)
